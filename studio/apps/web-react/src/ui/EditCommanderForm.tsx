...
export function EditCommanderForm({ controller, presenter }: Props) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (form) await controller.submit(form); };
      ...
      <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-slate-900/65 shadow-2xl shadow-black/10">
        <div className="flex items-start gap-4 border-b border-white/[0.06] px-6 py-5">
          ...
          <div>
            ...
            <h2 className="mt-0.5 text-lg font-semibold tracking-tight text-white">Modifier un commandant</h2>
            <p className="mt-1 text-sm leading-6 text-slate-400">Choisissez un commandant existant pour éditer sa configuration.</p>
          </div>
        </div>
        <div className="p-6">
          {commanders.length === 0 ? (
            <p className="text-sm text-slate-500">Aucun commandant dans le catalogue.</p>
          ) : (
            <Field label="Commandant à modifier">
              <select className={inputClass} value={selectedId} onChange={(e) => selectCommander(e.target.value)}>
                <option value="">Sélectionner un commandant…</option>
                {commanders.map((c) => <option key={c.id} value={c.id}>{c.name} ({c.id})</option>)}
              </select>
            </Field>
          )}
        </div>
      ...
      {form && (
        <form className="space-y-5" onSubmit={submit}>
          <Section eyebrow="Profil" icon="identity" title="Identité du commandant" description="Informations qui identifient le commandant dans le jeu et dans les données partagées.">
            <div className="grid grid-cols-2 gap-5">
              <Field label="Identifiant">
                <input className={`${inputClass} opacity-50`} disabled value={form.id} />
              </Field>
              <Field label="Nom" required>
                <input className={inputClass} value={form.name} onChange={(e) => setField('name', e.target.value)} />
              </Field>
              <Field label="Description" className="col-span-2">
                <textarea className={`${inputClass} min-h-20 resize-y leading-6`} value={form.description} onChange={(e) => setField('description', e.target.value)} />
              </Field>
              <Field label="Icône">
                <input className={inputClass} value={form.icon} onChange={(e) => setField('icon', e.target.value)} />
              </Field>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-amber-400/15 bg-gradient-to-r from-amber-400/[0.07] to-transparent p-5">
            <div>
              <p className="text-sm font-semibold text-slate-200">Mettre à jour ce commandant ?</p>
              {viewModel.message && (
